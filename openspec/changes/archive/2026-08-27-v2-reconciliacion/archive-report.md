...
- **Test layer**: skipped — documental [REV] change (no source, no tests; node-pty binding unavailable in env). Documented as environmental limitation, NOT a change failure.
...
- **Preserved**: CAS de interacciones and XII.3 multiusuario remain explicitly deferred.

## Criteria Compliance (final)
| Criterion | Status | Evidence |
|-----------|--------|----------|
| F-01 Terminal/PTY implemented surface | ✅ COMPLIANT | XII.1 + §7 `terminal/*` row `(opcional, implementado)`; PTY real / attach humano / presencia |
...
| F-04 Confined JSONL debt | ✅ COMPLIANT | XII.4 one-line + §7 `Deuda confinada:` → `v2-adapter/F-04` |
...
- File-wide `grep fallback` still matches IV.1 / VI.3 / schema harness (legitimate candidate fallback, not transport). Scoped verification (VII.4 / §7 Transporte) used; future reviewers must scope likewise.

## Next Recommended
`v2-no-regresion` (bundle / manifiesto / admisión no-regression per F-02).

## Key Learnings
1. v2-reconciliacion closed as a fully documental [REV] change with a byte-identical mechanical archive copy.
