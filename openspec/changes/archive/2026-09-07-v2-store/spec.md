...
Complete the reference store with remaining DDL, repositories, interchangeable backends, and atomic idempotent migration.

## Constraints

- The store MUST remain pure Go/no cgo without new dependencies. `database/sql` and `modernc.org/sqlite` MUST NOT be imported outside `internal/store`; `internal/execution/report.go` MUST use store errors.
- This change SHALL create only exact §2 `leases` and `interactions`. `leases` MUST retain its composite primary key without FK or CHECK. `interactions` MUST retain its attempt FK, `permission|question` and `pending|resolved` CHECKs, and `UNIQUE(attempt_id, idempotency_key)`.
- `path_claims`, `attempt_transport`, `dag_hash`, and `base_commit` MUST be verified and MUST NEVER be redefined by this change.
- WAL and foreign-key enforcement MUST apply to every SQLite connection. Persisted timestamps MUST be RFC3339 UTC.
- Both backends MUST normalize failures to `ErrNotFound`, `ErrCheckViolation`, `ErrUniqueViolation`, and `ErrForeignKeyViolation`. The fake MUST NOT import a driver.
