# v2-reporte Specification

## Purpose

Define terminal execution change reporting: capture of an immutable committed starting point at execution creation, one execution-scoped on-demand changed-files report, deterministic canonical output that matches real Git, and a CLI-direct `haro report` surface ready for a future thin `execution.report` wrapper.

## Constraints

Reporting MUST use only read-only Git inspection and MUST NOT mutate repository state; pure Go/no-cgo operation MUST be preserved without new dependencies, while use of the Git binary is permitted. At `CreateExecution` the system MUST capture `git rev-parse HEAD` in the resolved repository root as nullable `executions.base_commit`; pre-existing dirty changes MUST NOT alter this committed anchor, existing rows MAY remain null, and `v2-reporte` SHALL solely own the idempotent, `PRAGMA table_info`-guarded column migration (`v2-store` MUST NOT diverge). A terminal top-level execution MUST expose one execution-scoped, side-effect-free report containing `changed_files`; it SHALL be computed on demand and MUST NOT be persisted in v1, and no per-step or nested-workflow reports MUST be produced. The report MUST equal the union of tracked `git diff --name-only --diff-filter=ADMR --find-renames` results and `git ls-files --others --exclude-standard`, MUST include new/untracked, modified, deleted, and renamed files with a rename emitting only its new path, and MUST return lexicographically sorted, de-duplicated canonical repository-relative paths with `.haro/**` excluded and any `../` escape failing closed. Shared mode MUST diff at repository root; isolated mode MUST use an existing `workspace_root` and SHALL fall back to a repository-root diff from `base_commit` after terminal removal, scanning untracked files only while the workspace exists; report behavior MUST NOT change worktree-removal timing. `haro report <execution_id> [--json]` MUST delegate to the execution report; plain output SHALL contain one changed path per line, JSON MUST be `{execution_id, base_commit, changed_files}`, and the engine operation SHALL remain suitable for a future thin `execution.report` wrapper.
